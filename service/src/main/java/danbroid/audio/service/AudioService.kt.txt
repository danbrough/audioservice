...
class AudioService : MediaSessionService() {
  ...
  override fun onCreate() {
    ...
    log.debug("created player: $exoPlayer")
...
//    addSession(session)
  ...
  override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
    log.debug("onStartCommand() hashCode:${hashCode()}")
    ...
    */
    ...
    exoPlayer.addListener(object : Player.Listener {
      ...
      override fun onTracksChanged(
        ...
        log.trace("onTracksChanged()")
      }

      override fun onMetadata(metadata: Metadata) {
        if (BuildConfig.DEBUG)
          (0 until metadata.length()).forEach {
            log.trace("onMetadata() ${metadata[it]}")
          }
      }
  ...
  override fun onUpdateNotification(session: MediaSession): MediaNotification? {
    log.trace("onUpdateNotification()")
  ...
  override fun onGetSession(controllerInfo: MediaSession.ControllerInfo): MediaSession {
    log.trace("onGetSession() controllerInfo: $controllerInfo")

    return session
  }

  private fun loadIcon(mediaItem: MediaItem) {
    log.trace("loadIcon() $mediaItem")
  ...
  inner class ServiceNotificationListener() : PlayerNotificationManager.NotificationListener {
    ...
    override fun onNotificationPosted(
      ...
      log.trace("metadata: ${session.player.currentMediaItem?.metadata.toDebugString()}")
      ...
      log.trace("dominantColor: $dominantColor duration:${session.player.currentMediaItem.duration}")
    ...
    override fun onCustomCommand(
      ...
      if (customCommand.customAction == ACTION_ADD_TO_PLAYLIST) {
        ...
        session.player.addPlaylistItem(Int.MAX_VALUE, mediaItem).then {
          log.trace("addToPlaylistItem returned ${it.successfull}")
        }
...
//      val currentMetadata = player.currentMediaItem!!.metadata!!
      ...
      /*  val oldMetadata = session.player.playlistMetadata?.also {
          log.trace("using playlistMetadata")
        } ?: session.player.currentMediaItem?.metadata?.also {
          log.trace("using currentMediaItem metadata")
        }!!
        ...
        if (oldTitle != title && title != null) {
          MediaMetadata.Builder(oldMetadata)
              .putString(MediaMetadata.METADATA_KEY_DISPLAY_SUBTITLE, title)
              .build().also {
                log.trace("updating metadata with new title: $title")
          ...
          for (n in 0 until trackGroups.length) {
            for (m in 0 until trackGroups[n].length) {
              trackGroups[n].getFormat(m).also { format ->
                ...
                val currentMetadata = player.currentMediaItem?.metadata
                log.trace("ANALYTICS:bitrate: ${format.bitrate}")
                log.trace("ANALYTICS:trackMetadata:$n cls:${format::class.java} ${metadata}")
                log.trace("ANALYTICS:currentMetadata: ${currentMetadata}")
                ...
                if (metadata != null) {
                  (0 until metadata.length()).forEach {
                    val entry = metadata.get(it)
                    log.trace("ANALYTICS: entry: ${entry} cls: ${entry::class.java}")
                    when (entry) {
                      is VorbisComment -> {
                        log.trace("ANALYTICS:VORBIS COMMENT: ${entry.key}:=<${entry.value}>")
                        when (entry.key) {
                  ...
                  if (currentMetadata != null && title != null) {
                    ...
                    if (oldTitle != title) {
                      ...
                      log.trace("ANALYTICS: updating currentMediaItem metadata with title:$title")
